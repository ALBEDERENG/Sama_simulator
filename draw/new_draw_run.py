...
#Para executar no cmd, ative o ambiente que possui as seguintes libs, mude o diretório para da pasta draw e execute o comando: python new_draw_run.py
...
def validate_config(config):
    ...
    if config['general']['make'] not in ['graph']:
        errors.append("Invalid option in general.make")

    if config['graph']['x_var'] == '':
        errors.append("graph.x_var is empty")

    if config['graph']['y_var'] == '':
        errors.append("graph.y_var is empty")

    if errors:
        print("\nCONFIGURATION ERRORS:")
        for e in errors:
            print(" -", e)
        print("\nPlease edit config.yml!")
        sys.exit()
